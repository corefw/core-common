...
class Directory extends Core.cls( "Core.fs.BaseEntity" ) {
	...
	async hasChild( searchFor, allowFiles = true, allowDirectories = true ) {
		...
		// Get my contents
		let contents = await me.getSimpleContents( true );
	...
	async getSimpleContents( returnRelPaths = false ) {
		return this.fs.getDirectoryContentsSimple( this.path, returnRelPaths );
	}
