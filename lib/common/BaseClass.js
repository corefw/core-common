...
class BaseClass {
	...
	$dep( name, cfg ) {
		...
		if ( TIPE( name ) === "string" ) {
			...
			DEP = me.constructor.$dep( name );

			// not found, look in dependency map...
	 ...
	 * @returns {Object} The file contents.
	 */
	loadYamlFile( absPath, property ) {

		const me = this;

		return me.constructor.loadYamlFile( absPath, property );
	}
	...
	/**
	 * Convenience alias for the `logMetric` method of the `Logger` class.
	 *
	 * @public
	 * @param {string} type - The type of metric to log; can be 'count', 'gauge', or 'timing'
	 * @param {number} value - The value of the metric
	 * @param {string} [name] - The name of the metric
	 * @param {string} [messageMask] - A masked used to generate a log message that includes the value ("{{value}}").
	 * @param {Object} [params] - Additional variables to be included in the log output and injected into the messageMask.
	 * @returns {?Object} The log entry created, or NULL if no Logger instance is available.
	 */
	$metric( type, value, name, messageMask, params ) {

		const me = this;

		if ( me.logger !== null ) {

			return me.logger.logMetric( type, value, name, messageMask, params );
		}
	...
	$count( value, name, messageMask, params ) {
		...
		return this.$metric( "count", value, name, messageMask, params );
	}
	...
	$timing( value, name, messageMask, params ) {
		...
		return this.$metric( "timing", value, name, messageMask, params );
	}
	...
	$gauge( value, name, messageMask, params ) {
		...
		return this.$metric( "gauge", value, name, messageMask, params );
	}

	/**
	 * A convenience method for throwing simple errors, with Logger support.
	 *
	 * @protected
	 * @param {string} id - An error/message id
	 * @param {string|Error} error - Either a simple error message (string) or an Error object.
	 * @param {string?} prefix - An optional error prefix (will be prepended to the error message with a colon)
	 * @returns {void}
	 */
	$throw( id, error, prefix ) {

		const me = this;

		// Allow for an error object or a string.
		if ( error.message === undefined || error.message === null ) {

			error = new Error( String( error ) );
		}

		// If a prefix is provided, we'll use the VError library
		// to create a new error object..
		if ( prefix !== undefined && prefix !== null && prefix !== "" ) {

			const verr = me.$dep( "verror" );

			error = new verr.VError( error, prefix );
		}

		// If we have a logger object, we'll send the error to it, first..
		if ( me.logger !== null ) {

			me.logger.log( "critical", id, error.message );
		}
	 ...
	 * @param {?number} [indent=0] - An optional amount to indent the output,
	 *     using tabs.
	 * @returns {string} A string showing the contents of `varToInspect`.
	 */
	...
	/**
	 * Outputs an abbreviated/summary version of `this.$config` to the
	 * attached logger via `this.$log`.
	 *
	 * @access protected
	 * @uses #$inspect
	 * @uses #$log
	...
	$logConfig() {
		...
		// Dependencies
		const _		= me.$dep( "lodash" );
		const TIPE	= this.$dep( "tipe" );

		// Translate this.$config into a summary object that
		// is more appropriate for outputting to the logger.
		_.each( me.$config, function ( cfgValue, cfgItem ) {

			if ( TIPE( cfgValue ) !== "object" || cfgValue.constructor.name === "Object" ) {

				// Normal variables pass straight through
				cfgSummary[ cfgItem ] = cfgValue;

			} else {

				// Class instances will be converted to a short
				// string description (to avoid a massive amount of output)
				cfgSummary[ cfgItem ] = "{ Instance of Class: " + cfgValue.constructor.name + " }";
			}
		} );

		// Use the inspector to resolve a pretty string that
		// shows the contents of the summary object
		let output = me.$inspect( cfgSummary, false, 1 );

		// Send the final output to the logger
		me.$log( "info", "config.dump", "Dumping $config Object Summary ...\n\n" + output + "\n" );
	}
