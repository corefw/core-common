 ...
 * instances have a shared sequences counter.
 *
 * @memberOf Logging
...
class Logger extends BaseClass {
	...
	log( level, a, b, c ) {
		...
		// Override eventType
		if (
			logArguments.values.source !== undefined &&
			logArguments.values.source.type !== undefined
		) {
		...
		let logEntry = _.merge(
			{
				...
				application  : {
					...
					version   : me.getSetting( "version" ),
					stage     : me.getSetting( "stage" ),
	...
	_getLogLevels() {
		...
		if ( me.__logLevelCache === undefined ) {
			...
			_.each( me.__logLevelCache, function ( levelInfo ) {
	 ...
	 *
	 * @public
	 * @param {string|Object} type - Metric type or arguments object.
	 * @param {*|Object} value - Metric value or arguments object.
	 * @param {string|Object} [name] - The name of the log event and metric or arguments object.
	 * @param {string|Object} [messageMask] - Message mask content of log entry or arguments object.
	 * @param {Object} [params] - Arguments for formatting message mask.
	...
	logMetric( type, value, name, messageMask, params ) {
		...
		} else if ( TIPE( name ) === "string" ) {

			args.name = name;

		}

		if ( TIPE( messageMask ) === "object" ) {

			_.assign( args, messageMask );

		} else if ( TIPE( messageMask ) === "string" ) {

			args.mask = messageMask;

		}

		if ( TIPE( params ) === "object" ) {

			args.values = _.assign( {}, params );

		}

		// Validate/coerce the metric type
		if ( args.type !== "timing" && args.type !== "gauge" ) {
		...
		// Format the name
		args.name		= me._formatLogMessage( args.name, args.values );
		args.metricId	= args.name;

		// If we do not have a message mask, we will
		// default the message to the name of the metric
		if ( _.isNil( args.mask ) ) {
		...
		if ( args.mask.indexOf( "{{value}}" ) !== -1 ) {

			args.mask = args.mask.replace( /{{value}}/ig, "{{metric.value}}" );

		}

		// Ensure the metric value exists somewhere in the message mask
		if ( args.mask.indexOf( "{{metric.value}}" ) === -1 ) {

			args.mask = "{{metric.value}} " + args.mask;

		}

		// Surround {{metric.value}} in braces..
		args.mask = args.mask.replace( /{{metric.value}}/ig, "[{{metric.value}}]" );

		// Apply "ms" suffix for timing metrics
		if ( type === "timing" ) {

			args.mask = args.mask.replace( /{{metric.value}}/ig, "{{metric.value}}ms" );
		...
		if ( args.name === undefined || args.name === null ) {
			...
			throw new ERRORS.InvalidArgumentError(
				"Missing name argument in Logger#logMetric()"
			);
	...
	timingStart( name ) {
		...
		let timings = me.getSetting( "timings" );

		timings[ name.name || name ] = new Date().getTime();
	...
	timingStop( name, messageMask, params ) {
		...
		let value = new Date().getTime() - timings[ name.name || name ];
	 ...
	 * @param {Object} levelInfo - Level information.
	 * @param {Object} logEntry - Log entry.
	 * @param {boolean} [excludeLogId=false] - When TRUE, the log id will NOT
	 *     be appended to the end of the log output.
	 * @returns {void}
	 */
	_outputLinear( message, levelInfo, logEntry, excludeLogId ) {
		...
		// Cast 'excludeLogId' param
		if ( excludeLogId !== true ) {
		...
		if ( message.indexOf( "\n" ) !== -1 ) {
			...
			_.each( spl, function ( line, index ) {
				...
				let excludeLogIdForLine = false;

				if ( index !== 0 ) {

					excludeLogIdForLine = true;

				}

				me._outputLinear( line, levelInfo, logEntry, excludeLogIdForLine );
		...
		if ( logEntry._moment !== undefined ) {
			...
			// delete logEntry._moment;  <-- messes up log output for multi-line entries
		...
		str += chalk[ severityColor ](
			...
			" [ " + _.padEnd( chalk.green( logEntry.application.component ), 35 ) + " ] "
		);
	 ...
	 * @instance
	 * @private
	 * @returns {Object} The parsed log arguments.
	 */
	_parseLogArguments() {
