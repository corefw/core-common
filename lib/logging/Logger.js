...
class Logger extends BaseClass {
	...
	log( level, a, b, c ) {
		...
		// Format message
		let eventMessage = me._formatLogMessage( messageMask, eventValues );

		// Combine message with params and apply log field defaults
		let dt = moment.utc();

		let logEntry = _.extend(
			{
				"@timestamp"        : dt.format(),
	...
	_displayConfigWarnings() {
		...
		if ( me.getSetting( "application" ) === "unknown-application" ) {
			...
			me.warn(
				"c2cs-lib-logging: No application name set ('application').\n" +
		...
		if ( me.getSetting( "inspect" ) === true ) {
			...
			me.warn(
				"c2cs-lib-logging: Inspection mode is enabled ('inspect').\n " +
		...
		if ( me.getSetting( "linear" ) === true ) {
			...
			me.warn(
				"c2cs-lib-logging: Linear output mode is enabled ('linear').\n" +
	...
	_getAwsConfig() {
		...
		return {
			...
			sqsQueue        : process.env.LOGGER_AWS_QS_QUEUE,
		};
	}
	...
	logMetric( type, value, name, metricName, messageMask, params ) {
		...
		if ( !( metricName && TIPE( metricName ) === "string" ) ) {
	 ...
	 * @param {string} [name.metricName] - Metric name of log entry.
	...
	timingStop( name, metricName, messageMask, params ) {
		...
		return me.timing( value, name, metricName, messageMask, params );
	}
	...
	timing( value, name, metricName, messageMask, params ) {
		...
		return me.logMetric(
			"timing",
			value, name, metricName, messageMask, params
		);
	}
	...
	_formatLogMessage( messageMask, logValues ) {
		...
		result = template( {
			data: _.mapValues( logValues, function ( value ) {
