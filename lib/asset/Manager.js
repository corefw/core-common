...
class Manager {
	...
	_getClassByName( className ) {
		...
		if( !me.classExists( className ) ) {
			throw new VERROR( "Could not resolve the class definition for '" + className + "'; it was not found at the resolved path '" + classPath + "'" );
		}
		...
		try {
			...
			throw new VERROR( err, "Could not resolve the class definition for '" + className + "'" );
		}
	 ...
	 * confirmed.
	 * @returns {?string} Either the class name of the file at `absPath` or NULL if the class name could not be resolved.
	 */
	reverseLookup( absPath, opts = null ) {
		...
		let valid = Core.validator.validate( ret, {
			throwErrors     : false,
			isCoreClassName : true
		} );
