...
class Parenting {
	...
	$spawn( fullClassName, config ) {
		...
		if( me.classLoader === undefined || me.classLoader === null ) {
			me.$throw( "DependencyPrecedenceError", "Core.asset.mixin.Parenting::$spawn() was called prior to Core.asset.mixin.Parenting::$construct().  This was likely caused by a mixed class using $spawn() within its $construct() method, which should be avoided. Consider moving the $spawn() call to the $ready() method or to any other method that is called after class initialization." );
		}
