...
class Container {
	...
	resolve( name ) {
		...
		// Error if not found..
		if( !me.has( name ) ) {
			throw new Error( "Core.asset.ioc.Container: Attempted to resolve an unknown class dependency ('" + name + "')." );
		}
	...
	resolveMany( names ) {
		...
		if( Array.isArray( names ) ) {
			...
			throw new Error( "Invalid type for parameter `names` in Core.asset.ioc.Container#resolveMany().  An array, a Set, or a plain object was expected, but a value with type '" + TIPE( names ) + "' was provided." );
		}
