...
class Validator extends BaseClass {
	...
	_initialize( cfg ) {
		...
		me.formats = {
			"fixed(5,1)": {
				type     : "string",
				validate : /^-?[0-9]{1,4}(.[0-9])?$/,
			},
			"fixed(5,2)": {
				type     : "string",
				validate : /^-?[0-9]{1,3}(.[0-9]{1,2})?$/,
			},
			"fixed(5,3)": {
				type     : "string",
				validate : /^-?[0-9]{1,2}(.[0-9]{1,3})?$/,
			},
			"fixed(5,4)": {
				type     : "string",
				validate : /^-?[0-9](.[0-9]{1,4})?$/,
			},
			"fixed(5,5)": {
				type     : "string",
				validate : /^-?0(.[0-9]{1,5})?$/,
			...
			"fixed(10,1)": {
				type     : "string",
				validate : /^-?[0-9]{1,9}(.[0-9])?$/,
			},
			"fixed(10,2)": {
				type     : "string",
				validate : /^-?[0-9]{1,8}(.[0-9]{1,2})?$/,
			},
			"fixed(10,3)": {
				type     : "string",
				validate : /^-?[0-9]{1,7}(.[0-9]{1,3})?$/,
			},
			"fixed(10,4)": {
				type     : "string",
				validate : /^-?[0-9]{1,6}(.[0-9]{1,4})?$/,
			},
			"fixed(10,5)": {
				type     : "string",
				validate : /^-?[0-9]{1,5}(.[0-9]{1,5})?$/,
			},
			"fixed(10,6)": {
				type     : "string",
				validate : /^-?[0-9]{1,4}(.[0-9]{1,6})?$/,
			},
			"fixed(10,7)": {
				type     : "string",
				validate : /^-?[0-9]{1,3}(.[0-9]{1,7})?$/,
			},
			"fixed(10,8)": {
				type     : "string",
				validate : /^-?[0-9]{1,2}(.[0-9]{1,8})?$/,
			},
			"fixed(10,9)": {
				type     : "string",
				validate : /^-?[0-9](.[0-9]{1,9})?$/,
			},
			"fixed(10,10)": {
				type     : "string",
				validate : /^-?0(.[0-9]{1,10})?$/,
	...
	/**
	 * When TRUE, replace missing or undefined properties and items with the
	 * values from corresponding default keywords.
	 *
	 * @public
	 * @type {boolean}
	 * @default false
	...
	get useDefaults() {
		...
		return me.getConfigValue( "useDefaults", false );
	}

	// noinspection JSUnusedGlobalSymbols
	set useDefaults( /** boolean */ val ) {

		const me = this;

		me.setConfigValue( "useDefaults", val );
	}
	...
	_validateObject( data, schema ) {
		...
		// TODO: Find AJV equivalent or remove?
	...
	_createValidationError( ajvError ) {
		...
		// FIXME: figure out what this root param stuff is all about
		// It was breaking request body validation on missing /data/attributes/someSomeValue type errors.
		...
		switch ( ajvError.keyword ) {
			...
			case "required":
				...
				message =
					...
					"'" + ( ajvError.dataPath ? ajvError.dataPath + "/" : "" ) +
			...
			default:
				...
				if ( _.isEmpty( ajvError.dataPath ) ) {
