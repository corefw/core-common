/**
 * Defines the BaseGenerator class.
 *
...
class BaseGenerator extends BaseClass {
	...
	constructor( cfg ) {
		...
		if( !_.isPlainObject( cfg ) || !_.isString( cfg.serviceRootPath ) ) {
			throw new Error("All generators require the 'serviceRootPath' configuration property to be defined when being instantiated.");
		}

		// Store the absolute path to the target service
		this._serviceRootPath = cfg.serviceRootPath;

		// If a 'configVariant' was defined in the constructor config, save it..
		if( cfg.configVariant !== undefined && cfg.configVariant !== null ) {
			this._configVariant = cfg.configVariant;
		}

	}

	/**
	 * The absolute path to the common Serverless config directory in
	 * node-sls-tools, based on the generators current `configVariant` setting
	...
	get commonConfigRoot() {
		return PATH.join( this.toolsLibRoot, "serverless/config", this.configVariant );
	}

	/**
	 * The absolute path to the common schemas directory in
	 * node-sls-tools (`schemas`).
	 *
	 * @access public
	 * @var {string}
	 * @readonly
	...
	get commonSchemaRoot() {
		return PATH.join( this.toolsLibRoot, "schemas" );
	}

	/**
	 * The absolute path to the common schema definitions directory
	 * in node-sls-tools (`schemas/definitions`).
	 *
	 * @access public
	 * @var {string}
	 * @readonly
	...
	get commonSchemaDefinitionRoot() {
		return PATH.join( this.commonSchemaRoot, "definitions" );
	}
	...
	get serviceEndpointPath() {
		return PATH.join( this.serviceRootPath, "lib", "endpoints" );
	}
	 ...
	 */
	get configVariant() {
		if( this._configVariant === undefined || this._configVariant === null ) {
			return "default";
		} else {
			return this._configVariant;
		}
	}
	set configVariant( newVal ) {
		this._configVariant = newVal;
	}

	/**
	 * The path to the root directory of this library (node-sls-tools).
	 *
	 * @access public
	 * @var {string}
	 * @readonly
	...
	getServiceEndpoints( force ) {
		...
		if( force || me._endpointDetailsCache === undefined ) {
			...
			let regx = /serverless-function\.json/;
			let epConfigFiles = this._findFilesWithPattern( me.serviceEndpointPath, regx );
	...
	get functionDefaults() {
		...
		if( me._functionDefaults === undefined || me._functionDefaults === null ) {
			me._functionDefaults = me._loadCommonConfigFile("function-defaults.json");
		}
	...
	get httpEventDefaults() {
		...
		// Read from cache or load the config file..
		if( me._httpEventDefaults === undefined || me._httpEventDefaults === null ) {
			me._httpEventDefaults = me._loadCommonConfigFile("http-event-defaults.json");
		}
	...
	get httpSharedEventDefaults() {
		...
		// Read from cache or load the config file..
		if( me._httpSharedEventDefaults === undefined || me._httpSharedEventDefaults === null ) {
			me._httpSharedEventDefaults = me._loadCommonConfigFile("http-shared-event-defaults.json");
		}
	...
	get httpSharedEventScaffold() {
		...
		// Read from cache or load the config file..
		if( me._httpSharedEventScaffold === undefined || me._httpSharedEventScaffold === null ) {
			me._httpSharedEventScaffold = me._loadCommonConfigFile("http-shared-event-scaffold.json");
		}
	...
	_loadEpFunctionConfig( epDetailsObj ) {
		...
		if( !_.isPlainObject( epDetailsObj ) ) {
			throw new Error("Missing or invalid endpoint details object (epDetailsObj) in BaseGenerator#_loadEpFunctionConfig()");
		}
		...
		// Load each andpoint detail property (built prior) into
		...
		// `SeverlessConfigGenerator` class, but it was added here to allow
	 ...
	 *
	 * @access private
	 * @param {object} cfg A function configuration object (from `serverless-function.json`)
	...
	_applyEventDefaults( cfg ) {
		...
		_.each( cfg.events, function( eventWrapper, eventIndex ) {
	 ...
	 * the path will only be searched for within events of the specified type,
	 * or types (if an array is passed). If not provided, or NULL, then all
	 * event types will be searched.
	...
	_functionConfigHasPath( cfg, pathName, eventType ) {
		...
		if( eventType === undefined || eventType === null ) {
			...
			throw new Error("Invalid value passed for 'eventType' in BaseGenerator#_functionConfigHasPath().");

		}

		// Exit early if the cfg doesn't have any events..
		if( cfg.events === undefined || !_.isArray( cfg.events ) || cfg.events.length === 0 ) {
			return ret;
		}
		...
		_.each( cfg.events, function( evWrapper ) {
			...
			_.each( evWrapper, function( ev, evType ) {
				...
				if( eventTypes === null || eventTypes[ evType ] !== undefined ) {
	 ...
	 * @returns {object} The final object with all valid variable values injected.
	 */
	 ...
	 * @param {object} vars A plain object containing variables that can be
	 * injected into the object defined by the `obj` param.
	 * @returns {*} The variabled passed via the `target` parameter, but with
	 ...
	 * @param {object} vars A plain object containing variables that can be
	 * injected into the object defined by the `obj` param.
	 * @returns {*} The resolved variable, after variable values have been injected.
	 */
		...
		_.each( vars, function( varVal, varName ) {
			...
			if( ret.indexOf !== undefined ) {
				...
				if( _.isString( varVal ) ) {
					...
					if( ret.indexOf( varIdentifier ) !== -1 ) {
	 ...
	 * absolute path matches a given regular expression pattern (`regx`).
	 *
	 * @access protected
	 * @param {string} basePath The file system path to search
	 * @param {RegExp} regx The regular expression to test each absolute path against.
	 * @returns {object[]} A plain-object, keyed by absolute paths, that
	 * contains information about each file that was matched/found.
	 */
	_findFilesWithPattern( basePath, regx ) {

		let me = this;
		let allFiles = this._getFilesRecursive( basePath );
		let ret = {};

		_.each( allFiles, function( item, absPath ) {

			if( regx.test( absPath ) ) {
				ret[ absPath ] = item;
			}
	...
	/**
	 * Returns information about ALL items (including both files and directories)
	 * found after a recursive scan of the provided `absPathToRead`.
	 *
	 * @access protected
	 * @param {string} absPathToRead The file system path to scan/walk.
	 * @param {?object} [initialStore=null] This optional object is used
	 * to faciliate recursive scanning by providing a single object/ref
	...
	_getDirContentsRecursive( absPathToRead, initialStore ) {
		...
		let dirContents = this._getDirContents( absPathToRead );
	 ...
	 * @returns {object} An object, keyed by absolute path, that contains information
	 * about each item (dirs+files) found within the provided `absPathToRead`.
	 */
