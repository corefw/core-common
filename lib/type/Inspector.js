...
class Inspector extends Core.cls( "Core.abstract.Component" ) {
	...
	addSimpleCheck( checkOptions ) {
		...
		if( doExtraStuff ) {
			...
			// Create `failsFor` (if not provided), which is an inverse of `checksFor`
			if ( checkOptions.failsFor === null ) {
				checkOptions.failsFor = "non-" + checkOptions.checksFor;
			}
			...
			// Add a version of `failsFor` that is prefixed with an indefinite article.
			if ( me.isNil( checkOptions.failsForA ) ) {
				checkOptions.failsForA = A( checkOptions.failsFor );
			}
