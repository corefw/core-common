...
class Validator extends Core.cls( "Core.abstract.Component" ) {
	...
	$construct( checkCollection, assetManager ) {
		...
		if ( assetManager === null ) {
			...
			throw new Error(
				"Missing the `assetManager` class dependency for " +
				"`Core.type.Validator`; " +
				"this dependency is REQUIRED but was not provided!"
			);
	...
	executeCheck( checkName, value, additionalArgs = [] ) {
		...
		if ( !checks.has( checkName ) ) {
			...
			throw new Error(
				"Invalid check name ('" + checkName + "') passed to " +
				"Core.type.Validator::executeCheck(). " +
				"No such check exists in the attached `Core.type.check.Collection` object."
			);
	...
	_validateInstruction( value, instructions ) {
		...
		Core.throw(
			...
			"Invalid `instructions` passed to " +
			"`Core.type.Validator::_validateInstruction()`: " +
			"the provided value is not a valid, normalized, `ValidationInstruction`."
		);
	...
	_describeInstruction( instructions, curDepth = 1 ) {
		...
		throw new Error(
			"Invalid `instructions` passed to " +
			"`Core.type.Validator::_describeInstruction()`: " +
			"the provided value is not a valid, normalized, `ValidationInstruction`."
		);
	...
	_executeDescribeExpectationForCheck( checkName, negate, additionalArgs = [] ) {
		...
		if ( !checks.has( checkName ) ) {
			...
			throw new Error(
				"Invalid check name ('" + checkName + "') passed to " +
				"Core.type.Validator::_executeDescribeExpectationForCheck(). " +
				"No such check exists in the attached `Core.type.check.Collection` object."
			);
	...
	_normalizeValidationInstructions( instructions, curDepth = 1 ) {
		...
		if ( !me.isNil( instructions.$all ) ) {
			...
			if ( !me.isArray( instructions.$all ) ) {
				...
				throw new Error(
					"Invalid '$all' instruction provided to " +
					"'Core.type.Validator#_normalizeValidationInstructions()'; " +
					"an Array was expected but " + provided + " was provided."
				);
		...
		if ( !me.isNil( instructions.$any ) ) {
			...
			if ( !me.isArray( instructions.$any ) ) {
				...
				throw new Error(
					"Invalid '$any' instruction provided to " +
					"'Core.type.Validator#_normalizeValidationInstructions()'; " +
					"an Array was expected but " + provided + " was provided."
				);
		...
		if ( !me.isNil( instructions.$and ) ) {
			...
			if ( !me.isArray( instructions.$and ) ) {
				...
				throw new Error(
					"Invalid '$and' instruction provided to " +
					"'Core.type.Validator#_normalizeValidationInstructions()'; " +
					"an Array was expected but " + provided + " was provided."
				);
		...
		if ( !me.isNil( instructions.$or ) ) {
			...
			if ( !me.isArray( instructions.$or ) ) {
				...
				throw new Error(
					"Invalid '$or' instruction provided to " +
					"'Core.type.Validator#_normalizeValidationInstructions()'; " +
					"an Array was expected but " + provided + " was provided."
				);
