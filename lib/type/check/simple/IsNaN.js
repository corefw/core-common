/**
 * @file
 * Defines the Core.type.check.simple.IsNaN class.
 *
...
/**
 * A simple check that determines if a target variable is a 'NaN literal'.
 *
 * Note: This check is a thin wrapper for the Lodash function `isNaN` and its `evaluateTarget` method will
 * behave identically to that function.
 *
 * @memberOf Core.type.check.simple
 * @extends Core.type.check.simple.BaseSimpleCheck
 */
class IsNaN extends Core.cls( "Core.type.check.simple.BaseSimpleCheck" ) {
	...
	static get description() {
		return "Checks if a value is NaN (literally and exactly).";
	}
	...
	/**
	 * Checks if a `value` is NaN (literally and exactly).
	 *
	 * @static
	 * @public
	 * @param {*} value - The value to check.
	 * @returns {boolean} Returns TRUE if `value` is a nan literal; otherwise it returns FALSE.
	 */
