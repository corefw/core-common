/**
 * @file
 * Defines the Core.type.check.simple.IsSet class.
 *
...
/**
 * A simple check that determines if a target variable is a 'Set Object'.
 *
 * Note: This check is a thin wrapper for the Lodash function `isSet` and its `evaluateTarget` method will
 * behave identically to that function.
 *
 * @memberOf Core.type.check.simple
 * @extends Core.type.check.simple.BaseSimpleCheck
 */
class IsSet extends Core.cls( "Core.type.check.simple.BaseSimpleCheck" ) {
	...
	static get description() {
		return "Checks if a value is classified as a Set object.";
	}
	 ...
	 * @returns {boolean} Returns TRUE if `value` is a set object; otherwise it returns FALSE.
	 */
