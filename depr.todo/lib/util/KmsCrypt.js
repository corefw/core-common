...
class KmsCrypt extends BaseClass {
	...
	decryptObject( ciphertextData, config ) {
		...
		return BB.try( function () {
			...
			return YAML.safeLoad( plaintextData );
	...
	decryptFromS3( config ) {
		...
		return BB.try( function () {
			...
			let params = {
				...
				Key    : config.configFile || "kms-crypt.config.yml",
			};
			...
			kmsCryptConfig = YAML.safeLoad( data.Body.toString( "utf-8" ) );
	...
	encryptToS3( plaintextData, config ) {
		...
		return BB.try( function () {
			...
			// Backup existing ciphertext data...

			let timestamp = new Date().toISOString();

			let params = {
				Bucket     : config.s3Bucket,
				CopySource : config.s3Bucket + "/" + config.ciphertextFile || "ciphertext",
				Key        : ( config.ciphertextFile || "ciphertext" ) + "_" + timestamp,
			};
